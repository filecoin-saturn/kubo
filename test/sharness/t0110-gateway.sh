...
test_expect_success "GET IPFS path with explicit ?filename succeeds with proper header" "
  ...
  grep -iF 'Content-Disposition: inline; filename=\"test____.pdf\"; filename*=UTF-8'\'\''test%D1%82%D0%B5%D1%81%D1%82.pdf' actual_headers
"
...
test_expect_success "GET IPFS path with explicit ?filename and &download=true succeeds with proper header" "
  ...
  grep -iF 'Content-Disposition: attachment; filename=\"test____.mp4\"; filename*=UTF-8'\'\''test%D1%82%D0%B5%D1%81%D1%82.mp4' actual_headers
"
...
test_expect_failure "Verify gateway file" '
