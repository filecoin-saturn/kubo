...
test_expect_failure "subdomain gw: backlink on root CID should be hidden" '
...
test_expect_failure "subdomain gw: redirect dir listing to URL with trailing slash" '
...
test_expect_failure "subdomain gw: Etag should be present" '
  curl -sD - --resolve $DIR_HOSTNAME:$GWAY_PORT:127.0.0.1 http://$DIR_HOSTNAME:$GWAY_PORT/ą/ę/ > list_response &&
  test_should_contain "Index of" list_response &&
  test_should_contain "Etag: \"DirIndex-" list_response
'

test_expect_failure "subdomain gw: backlink on subdirectory should point at parent directory" '
  test_should_contain "<a href=\"/%C4%85/%C4%99/..\">..</a>" list_response
'

test_expect_failure "subdomain gw: breadcrumbs should leverage path-based router mounted on the parent domain" '
...
test_expect_failure "subdomain gw: name column should be a link to content root mounted at subdomain origin" '
  test_should_contain "<a href=\"/%C4%85/%C4%99/file-%C5%BA%C5%82.txt\">file-źł.txt</a>" list_response
'

test_expect_failure "subdomain gw: hash column should be a CID link to path router with filename param" '
