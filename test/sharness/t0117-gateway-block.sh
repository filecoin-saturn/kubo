...
# Make sure expected HTTP headers are returned with the block bytes
    ...
    cat curl_output &&
    grep -i "< Content-Type: application/vnd.ipld.raw" curl_output
    '
    ...
    grep -i "< Content-Length: $BYTES" curl_output
    '

    test_expect_success "GET response for application/vnd.ipld.raw includes Content-Disposition" '
    grep -i "< Content-Disposition: attachment\; filename=\"${FILE_CID}.bin\"" curl_output
    '
    ...
    grep -i "< Content-Disposition: attachment\; filename=\"foobar.bin\"" curl_output_filename
    '
...
# (basic checks, detailed behavior is tested in  t0116-gateway-cache.sh)
    ...
    grep -i "< Etag: \"${FILE_CID}.raw\"" curl_output
    '
